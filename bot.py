...
def add_channel_start(message):
    msg = bot.send_message(ADMIN_ID, "Forward any message from your channel.")
...
def finalize_channel(message, ch_id, ch_name):
    # Old logic for plan entry and demo link
    ...
    bot.register_next_step_handler(msg, save_channel_data, ch_id, ch_name, plans_dict)

def save_channel_data(message, ch_id, ch_name, plans_dict):
    ...
    db['channels'].update_one({"channel_id": ch_id}, {"$set": data}, upsert=True)
...
def show_plans_menu(chat_id, ch_id):
    ch_data = db['channels'].find_one({"channel_id": ch_id})
    if ch_data:
        ...
        markup.add(InlineKeyboardButton("📞 CONTACT ADMIN", url=f"https://t.me/{os.getenv('CONTACT_USERNAME')}"))
        bot.send_message(chat_id, f"WELCOME! You are joining: <b>{ch_data['name']}</b>\n\nPLEASE SELECT A PLAN:", reply_markup=markup, parse_mode="HTML")

@bot.callback_query_handler(func=lambda call: call.data.startswith('select_'))
def user_pays(call):
    bot.answer_callback_query(call.id)
    _, ch_id, mins = call.data.split('_')
    ch_data = db['channels'].find_one({"channel_id": int(ch_id)})
    ...
    if rz_client:
        try:
            ...
            transactions_col.insert_one({"user_id": call.from_user.id, "channel_id": int(ch_id), "minutes": int(mins), "amount": price, "status": "pending", "timestamp": datetime.now(timezone.utc), "order_id": order_id})
    ...
    markup.add(InlineKeyboardButton("🤳 MANUAL PAY (UPI QR)", callback_data=f"manual_{ch_id}_{mins}"))
    markup.add(InlineKeyboardButton("📞 CONTACT ADMIN", url=f"https://t.me/{os.getenv('CONTACT_USERNAME')}"))
    
    bot.send_message(call.message.chat.id, f"💎 <b>CHOOSE PAYMENT METHOD</b>\n\nPLAN: {format_clean_duration(mins)} @ ₹{price}", reply_markup=markup, parse_mode="HTML")
...
def receive_screenshot(message, ch_id, mins):
    ...
    markup = InlineKeyboardMarkup().add(
        InlineKeyboardButton("✅ APPROVE", callback_data=f"app_{user.id}_{ch_id}_{mins}"),
    ...
    bot.send_message(message.chat.id, "✔️ Proof sent! Please wait for admin approval.")
